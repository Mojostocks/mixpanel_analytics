...
const String COL_EVENT_NAME = 'event_name';
...
class Store {
  factory Store({String dbFile = DEFAULT_DB_NAME}) => _instances.putIfAbsent(dbFile, () => Store._(dbFile));
  ...
  Future<void> delete(List<int> eventIds) async {
    ...
    final count = await db.rawDelete('DELETE FROM $EVENTS_TABLE WHERE id IN (${eventIds.join(',')})');
    length -= count;
  }
  ...
  Future<int> _count(Database db) async {
    ...
    final List<Map<String, dynamic>> rows = await db.rawQuery('SELECT COUNT(*) as count FROM $EVENTS_TABLE');
  ...
  Map<String, dynamic> _serialize(Event e) {
    return <String, dynamic>{}
      ..[COL_EVENT_TYPE] = e.type
      ..[COL_EVENT_NAME] = e.name
      ..[COL_SESSION_ID] = e.sessionId
      ..[COL_TIMESTAMP] = e.timestamp
      ..[COL_PROPS] = json.encode(e.props);
  }

  Event _deserialize(Map<String, dynamic> map) => Event(
        map[COL_EVENT_TYPE],
        map[COL_EVENT_NAME],
